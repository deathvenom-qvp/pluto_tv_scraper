(function() {
	...
	const updateClientID = (filePath) => {
		let contents = fs.readFileSync(filePath, 'utf-8');
		const lines = contents.split('\n');
		contents = lines.map(line => {
			if (line.includes('clientID=00000000-0000-0000-0000-000000000000')) {
				const uuid = utils.generateUniqueClientID();
	...
	const process = async (config) => {
		...
		const getRegion = async (region) => {
			...
			try {
				...
				const clientID = utils.generateUniqueClientID();
				...
				updateClientID(`${outdir}/plutotv_${region}.m3u8`);
				...
				if (config.get('ondemand')) {
					...
					if (res?.m3u8) {
						fs.writeFileSync(`${outdir}/plutotv_ondemand_${region}.m3u8`, res.m3u8, 'utf-8');
						updateClientID(`${outdir}/plutotv_ondemand_${region}.m3u8`);
					}
